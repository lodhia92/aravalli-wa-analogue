...
# catchment cross-check key: sites sharing a HydroBASINS accumulated catchment area, split by
# spatial contiguity so that two separate basins of the same area are not merged.
cat = []
...
# two Mangalwar sites have no light rare-earth score (missing assays); they are excluded
# throughout, which is what the published quantile call did implicitly.
n_before = len(ind)
...
# published flags must be reproduced exactly
for dom in DOMAINS:
...
# the alternative fingerprint, for the letter only
