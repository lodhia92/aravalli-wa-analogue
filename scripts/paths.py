...
TECTONIC = os.path.join(DATA, "gswa", "GEOLOGY_500k_Tectonics_GDA2020_SHP", "ESRI",
                        "SHAPEFILES", "500k_tectonicp")
MINEDEX = os.path.join(DATA, "gswa", "Minedex_GDA2020_CSV", "Sites.csv")

_REQUIRED = {"NGSA": NGSA, "HMMA": HMMA, "NGCM_RAW": NGCM_RAW, "NGCM_TABLE": NGCM_TABLE,
             "HYBAS_AS": HYBAS_AS, "HYBAS_AU": HYBAS_AU, "MINEDEX": MINEDEX}


def require(*names):
    """Fail early, with the provider named, rather than deep inside a read."""
    missing = [(n, _REQUIRED[n]) for n in names
               if n in _REQUIRED and not os.path.exists(_REQUIRED[n])]
    if missing:
