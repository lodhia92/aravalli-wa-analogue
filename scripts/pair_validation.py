...
def build_pairs(aus_thr):
    ...
    mi, si = rin.mean(), rin.std(ddof=0)
    ma, sa = rau.mean(), rau.std(ddof=0)
    out = []
    for idf, adf, dom in [(sand, wapp, "Palaeoproterozoic"), (mang, yiln, "Archaean")]:
        zi = zlog_match(idf, mi, si)[MATCH].dropna()
        za = zlog_match(adf, ma, sa)[MATCH].dropna()
        Iv = zi.values
        Av = za.values
...
def spearman(a, b):
    ...
    p = (np.sum(null >= abs(rho)) + 1) / (NPERM + 1)
    return rho, p, len(a)
...
def main():
    ...
    ic = ngcm_all.set_index("sid")
    acn = ngsa_all.set_index("sid")
    robust = pd.concat(
        [
            P25[P25.domain == d].sort_values(["mnn", "dist"], ascending=[False, True]).head(10)
    ...
    rows = []
    for el in PATH:
        x, y = elem_vecs(robust, el)
        rho, p, n = spearman(x, y)
        rows.append(("elem", el, rho, p, n))
    for name, grp in MIN.items():
        x, y = elem_vecs(robust, None, grp)
        rho, p, n = spearman(x, y)
        rows.append(("mineral", name, rho, p, n))
    ...
    for thr in [15.0, 10.0]:
        Pt, _, _ = build_pairs(thr)
        sets.append((f"all-candidates-{int(thr)}%", Pt))
    crow = []
    for name, P in sets:
        rho, p, n = composite(P)
        crow.append((name, n, round(rho, 3), round(p, 3)))
        print(f"  {name:22} n={n:3d}  rho={rho:+.3f}  p={p:.3f}")
    pd.DataFrame(crow, columns=["pair_set", "n", "rho", "p"]).to_csv(
        f"{RES}/pair_composite_scaling.csv", index=False
    )
