...
def aus(path, label):
    return australian_pool(ng, pd.read_csv(path), label)
        ...
        for a in range(len(Av)):
            ...
            out.append(
                dict(
                    ...
                    dist=float(di[bi]),
                    dist_scaled=float(di[bi]) / np.sqrt(len(MATCH)),
                    mnn=mnn,
                )
            )
    P = pd.DataFrame(out)
...
def main():
    ...
    ar["Al"] = (
        ar["Al2O3"] * WT_PCT_TO_MG_KG * AL_MASS_FRACTION_OF_AL2O3
    )  # oxide wt% -> element mg/kg
    ...
    print(
        out.drop_duplicates("ratio_set")[
            [
                ...
                "n_candidates",
                "median_dist_scaled",
                "pairs_shared_with_original",
                "aus_sites_shared_with_original",
            ]
        ].to_string(index=False)
    )
    print("\n==== PER-MINERAL TRANSFER ====")
    print(
        out[["ratio_set", "mineral", "n_pairs", "rho", "p", "q", "transfers"]].to_string(
            index=False
        )
    )
